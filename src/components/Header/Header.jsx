    ...
    return (
        <>
            <header>
                ...
                {/* <BurgerMenu onOpenCard={onOpenCard}/> */}
                <ul className={styles.header__right}>
                    <li>
                        <button onClick={onOpenCard} className={styles.btn}>
                            <svg
                                width="20"
                                height="20"
                                viewBox="0 0 20 20"
                        ...
                        <span onClick={onOpenCard}>{getTotalPrice(cartItems)} руб.</span>
                    </li>
                    ...
                    <li>
                        ...
                        {favoritesItems.length > 0 && <div>{favoritesItems.length}</div>}
                    </li>
                    <li>
                        <Link to="/orders">
                            <button className={styles.btn}>
            ...
            <div
                className={`${styles.burgerMenu__container} ${burgerActive ? styles.visible : ''}`}
            >
                <div onClick={() => setBurgerActive(false)} className={styles.burgerMenu__bg} />
                <div
                    className={`${styles.burgerMenu} ${burgerActive ? styles.burgerVisibale : ''}`}
                >
                    <h3>REACT SNEAKERS</h3>
                    <Link onClick={() => setBurgerActive(false)} to="/">
                        Главная <FaHome />
                    </Link>
                    <Link onClick={handleOpenCart} to="/">
                        Корзина <FaCartShopping />
                    </Link>
                    <Link onClick={() => setBurgerActive(false)} to="/favorites">
                        Избарнное <MdFavorite />
                    </Link>
                    <Link onClick={() => setBurgerActive(false)} to="/orders">
                        Мои покупки <FaShoppingBag />
                    </Link>
                    <img onClick={() => setBurgerActive(false)} src="images/closeIcon.svg" alt="close button" />
                </div>
            </div>
        </>
    );
